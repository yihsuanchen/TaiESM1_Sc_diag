...
for station_index = 1%:num_stations - 1
    ...
    fname = sprintf('VOCALS_ST%02d_4scam_fixedgrid.nc', station_index);
    ...
    lev_data_orig = netcdf.getVar(vocals_allstations, netcdf.inqVarID(vocals_allstations, 'p'), [0,station_index-1, 0], [lev_mid_size,1,1]);
    lev_data = linspace(lev_data_orig(1),1000,numel(lev_data_orig))';

    netcdf.putVar(new_file, lev_varID, flip(lev_data));
    note_on_lev = 'Model pressure of the first time step only. Changed to fixed levels';
    ...
    for i = 1:numel(varnames_3d)
        ...
        var_data_orig = netcdf.getVar(vocals_allstations, netcdf.inqVarID(vocals_allstations, varname), [0, station_index-1, 0], [lev_mid_size, 1, time_size]);
        %[lev_mid_dimID_new ,time_dimID_new] = size(var_data_orig);
        var_data = zeros(lev_mid_size ,time_size);

        % Perform the interpolation for each timestep
        for t = 1:time_size
            % Interpolate the variable from sigma levels to target pressure levels for the current timestep
            var_data(:,t) = interp1(lev_data_orig, squeeze(var_data_orig(:, t)), lev_data, 'linear');
        end
